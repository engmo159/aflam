...
const NavBar = () => {
  ...
  const navList = (
    <ul
      className={`mt-2 mb-4 flex gap-2 lg:mb-0 w-fit lg:mt-0 lg:flex-row md:flex-row flex-wrap lg:items-center lg:gap-6 ${
        navScrolled
      ...
      <Typography
        ...
        <IconButton
          ...
          {theme === "dark" ? (
            <PiSunBold
              className={`${
                navScrolled ? "text-black dark:text-white" : "text-white"
              }`}
            />
          ) : (
    ...
    <>
      <Navbar
        className={`mx-auto max-w-screen-4xl px-4 py-2 border-none rounded-none z-50 fixed backdrop-saturate-100 ${
          navScrolled
            ? " dark:bg-gray-900"
            : "dark:lg:bg-transparent dark:backdrop-blur-none shadow-none "
        }`}>
        <div className="container mx-auto flex items-center justify-between text-white ">
          <div className="flex gap-x-4">
            <Link
              to="/"
              className="mr-4 cursor-pointer py-1.5 font-bold text-2xl text-black dark:text-white flex items-center">
              <img
                src={logo}
